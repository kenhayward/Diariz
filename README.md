...
| **Organise & merge** | Folders nested up to 8 levels deep with drag-and-drop; the list drills in one folder at a time with a breadcrumb collapsing the middle when it does not fit, each folder above the current one clickable to jump straight there, so it stays readable however many recordings you have; choose where a new recording is filed; browse as a list, calendar, cross-meeting actions, or tag cloud; merge recordings into one. |
