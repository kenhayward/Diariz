...
| **Organise & merge** | Folders nested up to 8 levels deep with drag-and-drop; the list drills in one folder at a time with a breadcrumb showing the full path, each part clickable to jump straight there, so it stays readable however many recordings you have; choose where a new recording is filed; browse as a list, calendar, cross-meeting actions, or tag cloud; merge recordings into one. |
