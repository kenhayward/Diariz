...
| **Record a calendar meeting** | Join a meeting from your calendar and it records in one click, named after the invite and linked to that meeting from the start (bringing any prep notes with it) rather than matched by time afterwards. Optionally let it end itself - a set number of minutes after the meeting was due to finish, or after a run of silence when everyone has left. Joining a second meeting finishes and files the first automatically. |
