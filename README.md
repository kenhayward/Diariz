...
| **Formulas** | Save a prompt and a chosen context, then run it over any recording - or over a whole folder and its sub-sections (every meeting in it) - to generate a named Markdown document (follow-up email, recap, and more) you can edit, download, or email. Runs happen in the background (the result shows "Generating..." then fills in). Built-in, platform-wide, and personal; share a personal one so others can find and add it (a live link) and run it; run one with `/formula <name>` in chat or ask Claude via MCP; admins manage the platform-wide and built-in formulas from a Manage Formulas window. |
