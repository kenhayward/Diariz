...
| **Multi-user & groups** | A **Users & access** console - search and filter accounts, and see at a glance what a person's groups let them do; groups grant the five platform permissions, each explained in a sentence; access requests get their own tab with a waiting count, then an approval lifecycle; per-user data isolation; sign-in lockout and per-address limits, and disabling an account also stops its API, MCP and connector tokens; Light/Dark/Auto themes. |
