...
| **Live transcript** | While you are still recording, the notes panel gains a **Transcript** tab that fills in as the meeting runs, so you can check what was said twenty minutes ago without waiting for the meeting to end - and the assistant can be asked about a meeting that is still going, and is told the conversation is unfinished so it does not report an argument still in progress as settled. The live text is provisional and says so: a status line shows how far behind it is, it deliberately carries **no speaker names** (a diarization label only holds within the half-minute it came from, so naming voices waits until it can be made stable across the whole meeting), and the full transcript replaces it when you press Stop. If the server falls too far behind it stops writing the live text and tells you, which never affects the recording itself. |
