...
/// shown name (a rename/identification, else the raw label); <see cref="PersonId"/>/<see cref="IdentifiedAuto"/>/
...
public record MergeSegmentInput(
    ...
    Guid? PersonId = null, bool IdentifiedAuto = false, bool IsMultiSpeaker = false);
...
public record MergedSpeaker(string Label, string DisplayName, Guid? PersonId, bool IdentifiedAuto, bool IsMultiSpeaker);
        ...
        foreach (var src in sources.OrderBy(s => s.Index))
        {
            foreach (var seg in src.Segments)
            {
                ...
                speakers.TryAdd(label, new MergedSpeaker(
                    label, seg.DisplayName, seg.PersonId, seg.IdentifiedAuto, seg.IsMultiSpeaker));
            }
