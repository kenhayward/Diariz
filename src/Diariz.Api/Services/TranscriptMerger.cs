...
/// shown name (a rename/identification, else the raw label).</summary>
public record MergeSegmentInput(string SpeakerLabel, string DisplayName, long StartMs, long EndMs, string Text);
...
public record MergedSpeaker(string Label, string DisplayName);
...
public static class TranscriptMerger
{
    public static TranscriptMergeOutput Merge(IReadOnlyList<MergeSourceInput> sources)
    {
        ...
        var speakers = new Dictionary<string, string>(); // namespaced label -> display name (first wins)
        long offset = 0;
        var ordinal = 0;
        ...
        foreach (var src in sources.OrderBy(s => s.Index))
        {
            foreach (var seg in src.Segments)
            {
                ...
                speakers.TryAdd(label, seg.DisplayName);
            }
            offset += src.DurationMs;
        }

        return new TranscriptMergeOutput(
            segments,
            speakers.Select(kv => new MergedSpeaker(kv.Key, kv.Value)).ToList());
    }
}
