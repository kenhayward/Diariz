...
public class SectionSummaryWorker : BackgroundService
{
    ...
    /// never runs, and the message stays pending to be abandoned again on the next pass - so the folder summary is
    ...
    private async Task AbandonEntryAsync(StreamEntry entry, CancellationToken ct)
    {
        var payload = entry["job"];
        if (!payload.HasValue ||
            JsonSerializer.Deserialize<SectionSummaryJob>((string)payload!) is not { } job) return;

        using var scope = _scopes.CreateScope();
        var ctx = scope.ServiceProvider.GetRequiredService<DiarizDbContext>();
        await SectionSummaryProcessor.AbandonAsync(ctx, _hub, job, _log, ct);
    }
