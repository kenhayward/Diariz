...
public static class SectionMinutesProcessor
{
    ...
    /// <summary>Settles a folder whose folder-minutes job was dropped without ever running - the reclaimer
    ...
    public static async Task AbandonAsync(
        DiarizDbContext db, IHubContext<TranscriptionHub> hub, SectionMinutesJob job, ILogger logger,
        CancellationToken ct = default)
    {
        var minutes = await db.SectionMinutes.FirstOrDefaultAsync(x => x.SectionId == job.SectionId, ct);
        if (minutes is null || minutes.Status != SectionGenerationStatus.Generating) return;

        var section = await db.Sections.FirstOrDefaultAsync(x => x.Id == job.SectionId, ct);
        if (section is null) return;

        logger.LogError(
            "Folder-minutes generation for section {SectionId} was abandoned by the queue; marking it failed",
            section.Id);
        minutes.Status = SectionGenerationStatus.Failed;
        minutes.Error = "Folder-minutes generation was abandoned after repeated failures. Try again.";
        await db.SaveChangesAsync(ct);
        await hub.NotifySectionStatusAsync(section.UserId, section.Id, "minutes", "Failed");
    }
