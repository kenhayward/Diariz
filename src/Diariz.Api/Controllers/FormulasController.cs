...
public class FormulasController : ControllerBase
{
    ...
    [HttpGet]
    [EndpointSummary("List the formulas you can run")]
    [EndpointDescription(
        "Everything available to you in one list: your own Personal formulas, every enabled shared " +
        "(Platform or Diariz built-in) formula, and any Personal formula someone else shared that you have " +
        "added. Disabled shared formulas are left out, because you could not run them anyway.\n\n" +
        "Each carries its `scope`, its template `content` (the blocks that build the document), a `context` " +
        "bitmask saying which parts of a meeting the model may see, and any Workflow Signals attached to it.")]
    ...
    [HttpGet("managed")]
    [EndpointSummary("List the shared formulas for administration")]
    [EndpointDescription(
        "Every Platform and Diariz built-in formula, **including disabled ones** - the admin view, as opposed " +
        "to the runnable list, which hides what you cannot use. Personal formulas never appear here, not even " +
        "your own. Requires the Manage Formulas platform permission (403 otherwise).")]
    ...
    [HttpPost]
    [EndpointSummary("Create a formula")]
    [EndpointDescription(
        "Saves a template: a `content` document of blocks (headings, literal text, substituted meeting " +
        "details, instructions to the model) plus a `context` bitmask choosing what the model may see - " +
        "transcript, notes, attachments, summary, minutes, actions. A bit outside that set is rejected with " +
        "400, so a client cannot store a context the runner would ignore.\n\n" +
        "A `Personal` formula is yours and always usable by you; set `shared` to let others discover and add " +
        "it. A `Platform` or `Diariz` formula is available to everyone and needs the Manage Formulas " +
        "permission (403 otherwise). Built-in status is never settable by a client - only the seeder grants it.")]
    ...
    [HttpPut("{id:guid}")]
    [EndpointSummary("Edit a formula")]
    [EndpointDescription(
        "Partial update - only the fields you send change, so omit the rest rather than echoing them back. " +
        "The scope cannot be changed here; a Personal formula stays personal.\n\n" +
        "Editing your own Personal formula is always allowed. A Personal formula that is not yours returns " +
        "**404 rather than 403**, so its existence is not leaked. A shared formula needs the Manage Formulas " +
        "permission. Documents already generated by this formula are left as they are - editing the template " +
    ...
    [HttpDelete("{id:guid}")]
    [EndpointSummary("Delete a formula")]
    [EndpointDescription(
        "Removes the template. Documents it already produced **survive** - they are stored text, not a live " +
        "view of the formula, so deleting one does not erase history.\n\n" +
        "Refused with 400 when a meeting type uses this formula to generate its minutes (the message names " +
        "them; point those types elsewhere first), and for a Diariz built-in, which can never be deleted. " +
        "Same permissions as editing, including the 404-not-403 rule for someone else's Personal formula.")]
    ...
    [EndpointSummary("Enable or disable a shared formula")]
    [EndpointDescription(
        "Turns a Platform or Diariz formula on or off for everyone - the reversible alternative to deleting " +
        "one. A disabled formula disappears from the runnable list but stays visible in the admin list, and " +
        "the documents it produced are untouched.\n\n" +
        "Only applies to shared formulas: Personal ones are always available to their owner, so this returns " +
        "400 for them. Requires the Manage Formulas permission.")]
    ...
    [HttpGet("shared")]
    [EndpointSummary("Browse formulas other people have shared")]
    [EndpointDescription(
        "The discovery list: Personal formulas **other users** have marked as shared, with each owner's " +
        "display name and avatar and whether you have already added it. Your own formulas are excluded, " +
        "shared or not.\n\n" +
        "This is a catalogue, not your collection - add one with the subscribe endpoint before it appears in " +
        "your runnable list. Open to any signed-in user; no permission needed.")]
    ...
    [EndpointSummary("Add a shared formula to your collection")]
    [EndpointDescription(
        "Adds someone else's shared Personal formula to your runnable list. This is a **live link, not a " +
        "copy**: the owner's later edits apply to you, and if they stop sharing or delete it, it leaves your " +
        "list. Idempotent - adding one twice is fine.\n\n" +
        "Returns 404 for a formula that is missing, not shared, not Personal, or already yours - one answer " +
        "for all of them, so you cannot probe for formulas you were not shown.")]
    ...
    [EndpointSummary("Remove a shared formula from your collection")]
    [EndpointDescription(
        "Drops your link to someone else's shared formula. Only the link goes - the owner's formula is " +
        "untouched, as are any documents it produced for you. Idempotent.")]
    ...
    [EndpointSummary("Run a formula over a recording")]
    [EndpointDescription(
        "Runs a formula against one meeting and produces a Markdown document. Returns 202 with the document " +
        "in `Generating`; watch its `status` (or the SignalR notification) for `Ready` or `Failed`, then fetch " +
        "the text.\n\n" +
        "An explicit run **replaces this recording's existing document for that formula** rather than adding a " +
        "second one, and overwrites it even if it had been edited by hand. To run one across a whole folder " +
        "instead, use the folder run endpoint.\n\n" +
        "Returns 404 for an unknown recording or formula, 403 for a formula you cannot run, and 400 when no " +
        "AI endpoint is configured for you or the platform.")]
