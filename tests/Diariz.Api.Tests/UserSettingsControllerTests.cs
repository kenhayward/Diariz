...
public class UserSettingsControllerTests
{
        ...
        IEnumerable<Diariz.Api.Tools.IChatTool>? tools = null, DictationOptions? dictation = null)
    {
        ...
        return new(db, Options.Create(chatOpts), toolResolver,
            new ChatContextResolver(db, Options.Create(chatOpts), new ChatModelCatalog(db, Options.Create(new LlmDefaultsOptions()))),
            new LlmSettingsResolver(db, Options.Create(new LlmDefaultsOptions()),
                Options.Create(server ?? new SummarizationOptions()), new FakeApiKeyProtector(), new ChatModelCatalog(db, Options.Create(new LlmDefaultsOptions()))),
