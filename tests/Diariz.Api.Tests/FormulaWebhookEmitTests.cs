...
public class FormulaWebhookEmitTests
{
    ...
    public async Task Successful_run_publishes_formula_result_completed()
    {
        ...
        var (_, owner, data, _, _, _) = publisher.Published.Single(p => p.EventType == WebhookEventTypes.FormulaResultCompleted);
    ...
    public async Task Successful_run_withAttachedActiveSignal_carriesSignalsAndPlatformData()
    {
        ...
        var (_, _, _, _, platformData, _) = publisher.Published.Single(p => p.EventType == WebhookEventTypes.FormulaResultCompleted);
