...
public class RecordingWebhookEmitTests
{
    ...
    public async Task Transcription_complete_publishes_recording_transcribed()
    {
        ...
        var (_, _, data, _, _, _) = publisher.Published.Single(p => p.EventType == WebhookEventTypes.RecordingTranscribed);
    ...
    public async Task Upload_publishes_recording_created()
    {
        ...
        var (_, _, data, _, _, _) = publisher.Published.Single(p => p.EventType == WebhookEventTypes.RecordingCreated);
        var json = System.Text.Json.JsonSerializer.Serialize(data);
        using var doc = System.Text.Json.JsonDocument.Parse(json);
