...
public class ActionsProcessorTests
{
    ...
    public async Task ProcessAsync_ExtractsActions_MarksExtracted_ReusesConfig_Notifies()
    {
        ...
        // The pipeline chains the minutes so they render this canonical action set.
        var minutesJob = Assert.Single(queue.MeetingMinutesEnqueued);
        Assert.Equal(rec.Id, minutesJob.RecordingId);
        Assert.Equal(tr.Id, minutesJob.TranscriptionId);
    }
    ...
    public async Task ProcessAsync_SkipsWhenAlreadyExtracted_DoesNotClobberUserEdits()
    {
        ...
        Assert.Single(queue.MeetingMinutesEnqueued); // minutes still (re)generated with the preserved actions
    }
    ...
    public async Task ProcessAsync_SkipsWhenLlmNotConfigured()
    {
        ...
        Assert.Single(queue.MeetingMinutesEnqueued); // minutes still chained (they gate on config themselves)
    }
    ...
    public async Task ProcessAsync_OnClientError_DoesNotThrow_LeavesStatusAndNoActions()
    {
        ...
        Assert.Single(queue.MeetingMinutesEnqueued); // minutes still chained even when extraction failed
    }
    ...
    public async Task ProcessAsync_NoSegments_DoesNothing()
    {
        ...
        Assert.Equal(0, client.Calls);
        Assert.Empty(await db.RecordingActions.ToListAsync());
        Assert.Single(queue.MeetingMinutesEnqueued); // minutes still chained (they no-op without segments)
    }
}
