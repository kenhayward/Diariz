...
| `Ordinal` | int | order within the transcription. Dense (0, 1, 2, ...) for a full-file pass; **sparse** for a provisional live one, where each chunk owns a band of 10,000 and its segments are numbered `Sequence * 10000 + i`. Only ever an `ORDER BY` key - never an index, identity or count - so the gaps are immaterial, and they are what keeps the per-chunk write cost independent of the meeting's length |
...
| `UsedMs` | int null | how much audio the last embedding actually consumed - the honest figure behind "using 1:20 of the 4:12 selected", since the worker caps pooled audio at `EMBED_MAX_SECONDS`. It used to double as the pending marker; it no longer does. A *failed* re-embed now leaves it alone rather than writing 0, which rendered as "trains on 0:00" - a confident figure for audio that was never measured |
| `RecomputeQueuedAt` | timestamptz null | a re-embed is queued and has not reported back. **Stored, not inferred.** It was previously derived as `SpansJson is not null && UsedMs is null`, which cannot express it: selecting the whole speaker serialises `SpansJson` to null, so the commonest case read as "not pending" and the UI said nothing when the button was pressed. `UsedMs` alone cannot stand in either - a sample enrolled straight from a speaker is never re-embedded, so it is null for almost every row (157 of 159 on the instance this was measured on) |
| `RecomputeFailedAt` | timestamptz null | the last re-embed failed. A failure keeps the existing vector - it must not destroy a working voiceprint - so without this the row is indistinguishable from one that never ran. Cleared by the enqueue and by a later success |
| `ConfirmedAt` | timestamptz null | a human listened and vouched that this recording really is this person. **A different assertion from `ExcludedAt`**: that asks whether the audio is good enough to learn from, this asks whether it is the right person, and a recording can be genuinely them and still be too noisy to train on. Takes it out of the review queue; multi-template voiceprints will gate template-seeding on it |
| `ConfirmedByUserId` | uuid null | who vouched. Recorded because the value of the gate is that a *named* human listened. **No FK** - the record of who asserted it must outlive their account, exactly as the sample outlives its recording |
