...
On the four content tabs - **Tags, Actions, Summaries, MinutesAndFormulas** - the test call runs the
**real prompt for that call group** against a **real recording of the administrator's own**, and the panel
shows the **parsed result** the pipeline would have produced.

The chosen recording is remembered per administrator, so the four tabs and successive models are all
tested against the same content.

`ModelBase`, `Translation` and `Chat` are unchanged and keep the fixed sample. ModelBase is a parameter
scope that nothing is ever dispatched to, so it has no real prompt. Translation needs a target language
and Chat needs a question; both are out of scope here.

## Decisions taken

| Decision | Choice | Why |
|---|---|---|
| Fixed sample vs recording | Recording **replaces** the fixed sample on the four content tabs | One button, one code path. The fixed sample survives only where there is no real prompt. |
| Tab coverage | Tags, Actions, Summaries, MinutesAndFormulas | The other three need inputs a recording does not supply. |
...
| Remembering | Per administrator, server-side, one recording shared by all four tabs | Survives browser and machine changes; one recording keeps timings comparable between models. |
...
| Minutes | One call, real template | Faithful prompt and output without turning one test into a dozen calls. Labelled as the single-call shape. |
...
- **TTFT is no longer comparable across models unless the same recording is remembered.** This is why
  remembering it is part of the feature rather than a nicety, and why the selection is shared by all four
  tabs rather than per tab.
- **The four content tabs stream while their real pipeline clients do not.** The prompt and the returned
...
- **The Minutes test is the single-call shape even when the platform is set to PerSection.** The panel
  says so. Reproducing a per-section run would make one test a dozen calls and make the timings describe a
  sequence rather than a call.
- **The Minutes test skips the notes-enhancement pre-pass** (a separate LLM call) and substitutes the note
  lines with `NotesComposer.RenderRaw` - the same fallback the real path uses when that call fails. Keeps
  the test to one call.
...
- The existing fixed prompt moves out of the probe into `LlmTestSample` (a `ChatMessage[]` constant),
  still used by ModelBase / Translation / Chat.
- `MaxResponseChars` stops being a constant: 8,000 for the fixed sample, 40,000 for recording-backed
  groups. A real minutes document legitimately exceeds 8,000 characters, and truncating it would produce a
  parse failure that the real pipeline would never hit.
...
| MinutesAndFormulas | `IMeetingTypeMinutesGenerator.BuildSingleCallMessagesAsync` | the meeting type's primary formula |
...
- `TagsPrompt`, `ActionsPrompt`, `SummarizationPrompt` and `MeetingMinutesPrompt` each grow a
  `ParseContent(string content, ...)`; the existing `ParseResponse(string responseJson, ...)` becomes a
  thin wrapper that pulls `choices[0].message.content` out and delegates. Pure extraction, no behaviour
  change, and it keeps one parsing implementation rather than two.

`LlmTestOutcome` gains three fields:

- `ParsedKind` - `"Tags" | "Actions" | "Summary" | "Minutes" | null` (null for the fixed-sample groups)
- `ParsedJson` - the parsed structure, serialised
- `ParseError` - the message when parsing threw

**The Minutes seam.** The single-call minutes prompt cannot be rebuilt from outside today:
`ResolveTemplateAsync`, `ResolvedTemplate`, `PrimaryContext` and `ResolveField` are all private to
`MeetingTypeMinutesGenerator`, and the message assembly lives inside `SingleCallMinutesStrategy`. Two
extractions, both behaviour-preserving:

1. `SingleCallMinutesStrategy` gains
   `public static Task<IReadOnlyList<ChatMessage>> BuildMessagesAsync(MinutesComposition input)`; its
   `GenerateAsync` calls it and then the client.
2. `IMeetingTypeMinutesGenerator` gains `BuildSingleCallMessagesAsync(...)`, sharing template resolution,
   context building and field resolution with `GenerateAsync`, forced to the single-call shape and using
   `NotesComposer.RenderRaw` instead of the notes pre-pass.

Neither widens visibility ad hoc: both are real seams the test path needs and the production path shares.
...
- 400 when the group is one of the four and no recording is given.
...
- On the four content tabs: the picker, pre-filled from the remembered recording, plus a one-line note
  that the transcript is sent to the endpoint under test. Choosing a recording PUTs it immediately.
- On ModelBase / Translation / Chat: static "Built-in sample transcript" text. Behaviour unchanged.
...
| Minutes | markdown via the existing `renderMarkdown` |
...
- `ParseContent` on each of the four prompt classes: happy path, and the malformed shapes each parser
...
- `SingleCallMinutesStrategy.BuildMessagesAsync`: the extracted messages match what `GenerateAsync` sends
  (assert against a recording fake client).
...
- `TestRail` shows the picker on the four tabs and the static text on the other three; Run is disabled with
  no recording.
...
**Verification.** Layout and overflow in the rail (a minutes document is long) are checked in the running
app with real measurements, not by asserting class names - jsdom computes no geometry.
...
- Translation and Chat tabs (need a target language and a question respectively).
- ModelBase (a parameter scope, not a call type).
- Testing against another user's recording.
- A per-tab remembered recording.
- Reproducing a PerSection minutes run.
