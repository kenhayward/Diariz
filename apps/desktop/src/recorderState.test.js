...
test("idle + ready shows the mic, system, and both record items, enabled", () => {
...
test("tooltip reflects the phase and source", () => {
  ...
  assert.equal(trayTooltip({ phase: "recording", source: "both", ready: true }), "Diariz — recording microphone + system audio");
...
test("notificationFor fires on start, on uploaded, and on error only", () => {
  ...
  assert.deepEqual(notificationFor({ phase: "idle" }, { phase: "recording", source: "both" }), {
    title: "Diariz",
    body: "Recording microphone + system audio…",
  });
  // upload finished
