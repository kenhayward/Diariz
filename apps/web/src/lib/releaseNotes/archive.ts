...
/// Every release covered by a closed epoch, newest first.
///
/// **Never import this module statically from application code.** It is the bulk of the release
/// history and is reached only through `loadArchive()` in the barrel, so it lands in its own lazy
/// chunk instead of the initial bundle. `releases.test.ts` enforces that; the tests themselves may
/// import it directly, because a test import does not affect the shipped graph.
