...
| **AI models** | A Platform Administrator configures every model the platform calls - its endpoint, key and context window - and how it is called: temperature, top P, top K, penalties, token caps, reasoning effort, timeout, and whether it supports tool calling or images. Each parameter can be inherited, set, or switched off so it is left out of the request. Parameters can differ per job (tag extraction, actions, summaries, minutes and formulas, translation, chat), a whole set can be copied between models, and each job can run on a different model with a default for the rest. |
