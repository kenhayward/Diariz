...
| **People and speaker identification** | One shared directory of the people in your meetings, where a voiceprint is optional - someone can be listed with no biometric held, or opt out, which erases theirs. Enrol a voice once and Diariz recognises it across later recordings (SpeechBrain voiceprints); rename, merge, and erase them (biometric data). Assign a speaker from the Speakers tab or straight from the label on any transcript row, as you read or listen. A **People** directory opens over whatever you are reading; it lists everyone one to a line, searchable by name, email or company; a person carries a job title, company, email, phone and an internal/external marker, and likely duplicates are pointed out for you to merge. The Speakers tab shows an identified speaker's title and company inline, with a contact card (email and phone as links) above their segments and a pencil to edit that person in place. Browsing the directory needs the Manage people permission; opting yourself out, and searching to name a speaker, never do. Your own entry is shown read-only on Preferences -> Profile - the name you appear under in transcripts, and whether a voiceprint is held for you. Each person's card has a **Voiceprint** tab listing every recording they appear in - not only the ones enrolled by hand - with how each was attributed, how much they speak in it, and tick boxes to add or drop it from training or to keep only certain segments, re-embedding from exactly what you picked. Play any segment to hear the voice behind the print; audio from a recording you do not own needs the Manage voiceprints permission, which plays only what that person said and logs every use. Where two people share a name, every list shows which Diariz account each one is - or says plainly that there is none - so a merge is decidable. When a voice is close but not certain, Diariz asks rather than guessing or staying silent - on the speaker in the transcript, and gathered under **Review Voice Matches**, a two-panel window listing the waiting voices beside what each one said, a line per segment with a play/stop button and a control to take it out, plus a Play all that runs through the kept segments highlighting each as it goes; excluding a segment keeps it out of the voiceprint, and voices whose audio has been deleted are not offered at all, since there is nothing left to listen to; a Platform Administrator sets how close is close enough, and can re-scan earlier recordings against the current settings. Each recording on that tab carries its own verdict - matches the others, a different recording condition, resembles nothing at all, or **sounds more like somebody else**, naming who - with the ones worth a listen sorted to the top and a tick box to show only those. Confirm a recording once you have listened and it leaves the review queue. Figures are stated as similarity, so a high number means a good match. Where a recording turns out to be somebody else, reassign it - or create that person, mark it as overlapping speech, or unlink it - from the same row, for recordings you own. In the directory, a person with a shaky voiceprint or a likely duplicate carries a short warning on their own row rather than a panel over the list, and a **Needs review** filter narrows to them. |
...
| **Meeting screenshots** | Capture the screen during a recording from the desktop app - a hotkey, the tray menu, or the app itself - choosing a screen or a rectangle on the first capture and reusing it after; or turn on **Auto-capture** and it takes one every time the screen settles on something new, capturing a presentation slide by slide on its own. Captures appear in the transcript at the moment they were taken, as a full-size viewer with zoom and pan to read a capture at native resolution, and in a Notes-tab section. From either the viewer's own **Add to chat context** button or by **dragging the thumbnail into the chat prompt**, a capture goes to a vision-capable model to ask about. Where an administrator has routed an **OCR** model, two more buttons read the text off a capture - into the chat prompt, or saved as a Markdown attachment on the meeting - cached so the second costs nothing, tables converted to Markdown so they render as tables, and always stamped with the model that read it and marked unverified. |
...
| **Google & calendars** | Optional Google sign-in and read-only Calendar linking, plus subscriptions to public iCalendar (.ics) feeds and - synced by the Windows desktop app on launch, from the tray or on demand - a mirror of your classic Outlook calendar (opt-in, per machine, erasable). All three are set up in one **Calendars** tab in Preferences, a card per source showing its account, its calendars and whether it is live. Every calendar you connect is treated the same: meetings from any of them show on the Calendar tab, match to a recording, and open with their full invite details. A month grid highlights the days that have something on them; picking one lays that day out on an hour axis, with meetings and recordings placed and sized by when they ran, clashes side by side, all-day entries in their own strip, and a line marking the current time. A **Go to today** button on the panel toolbar brings both the day and the month grid back to today after you have browsed ahead. Two toolbar buttons refresh it: **Sync calendar** covers every source you have connected in one go, and **Sync today** does the same for the current day only - seconds rather than the half a minute a full Outlook read takes - with the status bar counting up while either runs. |
| **LLM usage log** | Every model call recorded - who it was for, which model answered, how long it took, how many tokens, and why the model stopped. A **Cut off** badge flags a reply that hit its token cap, which otherwise looks identical to a model that answered nothing. Operations, Calls and Summary views with filters and a totals row over the whole filter. |
| **AI models** | A Platform Administrator configures every model the platform calls - its endpoint, key, context window and a one-line description people see when they pick it - and how it is called: temperature, top P, top K, penalties, token caps, reasoning effort, timeout, and whether it supports tool calling or images. Routing is a grid - models down the side, call types across the top, one click to move a call type to another model, a **No model** row for the ones that follow the default, and an **In chat** column marking which models people may pick between for chat. **Add all** adds every chat model on a server in one pass, leaving out embeddings and speech models and flagging any whose context window the server did not report. Parameters live in a drawer with a tab per call type, each parameter inherited, set, or omitted from the request entirely, beside a live preview of the exact request body that call type would send. **Run test** makes a real call with whatever is on screen and reports first-token time, duration, tokens per second and the model's reply - offering, on a failure, the single change that would fix it, and on any result a cURL command, the raw JSON, or a link into the usage log. For Tags, Actions and Summaries the test runs the **real prompt against one of your own recordings** (picked in the panel and remembered), and shows the parsed result the platform would have stored - weighted tags, an action table, a summary and its suggested name - saying so plainly when a reply would have yielded nothing. A whole set can be copied between models. |
...
export const RELEASES: Release[] = [
  ...
  },
  {
    ...
    date: "2026-08-24",
    pr: 597,
    headline: "Your personal room follows your name, and your profile shows who you are in transcripts",
    summary:
      "Renaming yourself updated your display name everywhere except your own room. A personal room takes its name from your display name when it is first created and then nothing ever changed it again, so an account renamed after the fact carried the old name in the room switcher indefinitely - on this deployment, the name it had been seeded with, months later. You cannot rename a personal room by hand, so there was no way to correct it either. It now follows your display name on every save, and rooms that had already drifted are corrected on upgrade.\n\nThe profile also tells you something it never has: who you are in the people directory. Every account is also a person - that is the record your voiceprint attaches to, and the name that appears against your speech in transcripts - but the directory that would show it needs the Manage people permission, so an ordinary user had no way to see their own entry at all, or to find out whether Diariz could recognise their voice. Preferences -> Profile now shows the name you appear under and whether you have a voiceprint, with a line on how to enrol one if you do not.",
    added: [
      "Profile: a \"You in transcripts\" panel showing the name you appear under and your voiceprint status.",
    ],
    fixed: [
      "A personal room kept the name it was created with when the account's display name changed.",
    ],
  },
  {
    version: "0.245.2",
  ...
  },
  {
    ...
    date: "2026-08-22",
    pr: 583,
    headline: "Extracted tables come through as tables, and the extract buttons show their progress",
    summary:
      "Three improvements to reading the text off a screen capture, all from using it in anger.\n\nThe models that read a page best answer with structure, not flat lines - a capture with a table on it comes back as an HTML table, and one model draws a gauge as an image tag. That is genuinely more useful than a wall of text, but until now it arrived as raw angle brackets: a page of `<table><tr><td>` in your chat prompt or your saved file. Tables are now converted into real Markdown tables, so they render as tables. Line breaks inside a cell survive, an image becomes its label, and text with no markup in it is passed through completely untouched.\n\nThe two extract buttons were labelled with text. They are now icons, and they tell you what is happening: the icon becomes a spinner while the text is being read - which can take a minute on the first run, while the model loads - and then a tick on whichever button did the work. Both buttons are disabled while a read is in flight, so a second impatient click cannot spend a second run on the same capture.\n\nAnd saving extracted text as an attachment now actually shows up: the Files tab was still listing the old set until you navigated away and back.",
    added: [
      "HTML tables in extracted text are converted to Markdown tables, so they render properly in a note or a chat prompt.",
    ],
    changed: [
      "The extract-text buttons are icons rather than text, and show a spinner while running and a tick when done.",
    ],
    fixed: [
      "The Files tab now refreshes after extracted text is saved as an attachment, instead of showing the old list until the page was revisited.",
    ],
  },
  {
    version: "0.241.1",
  ...
  },
  {
    ...
    date: "2026-08-22",
    pr: 580,
    headline: "Read the text off a screenshot, into the chat or into a file",
    summary:
      "A screen capture has always been a picture. You could send it to a vision model and ask about it, but the text on it was not text - you could not search it, quote it, or keep it once the capture was gone.\n\nOpen a capture full size and the toolbar now has two extract buttons. One reads the text off it and drops it into the chat prompt, ready to ask about. The other saves it as a Markdown attachment on the meeting, where it sits alongside your other files - renameable, editable, and pulled into chat with the rest of them. Both share one read: the result is kept on the capture, so choosing the second destination after the first costs nothing and returns instantly. Extract several captures and they accumulate in the chat pill rather than replacing each other; an uploaded file already in that slot is never overwritten without asking.\n\nThe existing \"Add to chat context\" button has not moved and does the same thing as before. Sending the picture to a vision model and reading the words off it are different jobs, and the right one depends on what you are asking.\n\n**Every extraction is stamped with the model that produced it and marked unverified, and that warning is meant literally.** Four OCR models were measured against one dense screenshot before this shipped. All four made silent mistakes: a misread letter, a whole table quietly dropped, and - at one image size - an entire column of neat, plausible scores that appears nowhere in the picture. Extracted text is a fast way to get at what a capture says. It is not a transcript, and anything that matters should be checked against the image.\n\nThis needs an administrator to route a model to the new **OCR** call type on the AI models page, exactly as Summaries or Tags are routed. Until one is, the buttons do not appear at all. Purpose-built OCR models are the ones worth using here - a general chat model asked to read a screenshot tends to describe it rather than transcribe it, and can invent a narrative wholesale.",
    added: [
      "Two extract-text buttons in the screenshot viewer: one sends the text to the chat prompt, the other saves it as a Markdown attachment on the meeting. Both are hidden unless an OCR model is routed.",
      "An OCR column on the AI models routing grid, picking which model reads screenshots - alongside Summaries, Tags, Translation and Chat.",
      "Per-model OCR prompt and maximum image size settings, because the right values differ sharply between models: measured on one capture, the best image size varied from 1288 to 2560 pixels, and going past a model's own best size made it worse rather than better.",
      "Extracted text is cached on the capture, so a second extraction of the same one is instant and costs nothing.",
    ],
    changed: [
  ...
  },
  {
    ...
    date: "2026-08-22",
    pr: 578,
    headline: "Send a screenshot to the chat from its own viewer, and delete it by accident far less often",
    summary:
      "Open a screenshot full size and the toolbar has been rearranged. The red delete button used to sit immediately to the left of the close button - same size, same X - so a click that missed Close by one button deleted the capture, with nothing to confirm and no way back. Delete now leads the right-hand group with a dividing line between it and everything else, so the routine controls are all on the far side of the fence.\n\nThe viewer also gained a speech-bubble button that attaches the capture you are looking at to the chat prompt. Until now the only way to ask a model about a screenshot was to close the viewer, find the thumbnail in the Notes tab, and drag it into the chat box - which is fiddly with a trackpad and impossible without a mouse. Click the button instead and the capture lands in the chat composer, exactly as a drag would: the button gains a tick to confirm it, the viewer stays open so you can keep reading the picture, and the chat panel opens itself if it was collapsed.\n\nEverything else about attached captures is unchanged - they ride every question until you remove them, and a model that cannot read images still says so before you send.",
    added: [
      "An \"Add to chat context\" button in the screenshot viewer's toolbar, attaching the capture on screen to the chat prompt without closing the viewer or dragging a thumbnail. The chat panel expands itself if it was collapsed.",
    ],
    changed: [
      "The screenshot viewer's delete button now leads the right-hand toolbar group, separated from the close, download and full-screen controls by a divider.",
    ],
  },
  {
    version: "0.239.0",
  ...
  },
  {
    ...
    date: "2026-08-21",
    pr: 573,
    headline: "Ask the assistant about a screenshot",
    summary:
      "Screen captures taken during a meeting have been visible for a while - in the transcript at the moment they were taken, and in the Notes tab - but the chat panel sitting next to them could read every word of the transcript and not a single pixel of the slide that was on screen. Asking \"what does this diagram show\" or \"read me the figures on that slide\" was the one thing it could not do.\n\nYou can now **drag a screenshot from the Notes tab straight into the chat prompt box**. It appears as a small thumbnail above where you type, and goes to the model with your question. Drop in as many as you like, one at a time; each thumbnail has an X in the corner to take it back out. They stay attached as you keep asking, so a follow-up question about the same image needs no second drag, and they are saved with the conversation - reopening it puts them back.\n\nReading images is something only some models can do, and your administrator marks which ones on the **Supports image input** setting for each model. Choose a model that cannot, and the Send button holds with **Select a vision model** next to it rather than quietly answering a question about a picture it never received. The model list marks the ones that can read images, so it is clear which to switch to.\n\nCaptures are shrunk to fit within 1920x1080 before they are sent, because the models read them more reliably at that size than at full 4K - a capture already smaller than that is sent exactly as it was taken. A very dense 4K screenshot may lose its smallest text to that resize.",
    added: [
      "Drag a screenshot from a recording's Notes tab into the chat prompt to ask a question about it. Attach several, one at a time, and remove any of them with the X on its thumbnail.",
      "Attached screenshots stay with the conversation as you keep asking, and are restored when you reopen a saved conversation.",
      "The chat model list marks which models can read images, and Send is held with \"Select a vision model\" when the chosen one cannot.",
    ],
    changed: [
      "The chat context dial now turns orange above 50% and red above 75%. It previously stayed blue until 90%, which left no warning while there was still time to act - so a conversation that reads as comfortable today may read as busier after this update, without anything having actually changed about it.",
      "An attached screenshot is counted in the context dial, so the gauge reflects what a picture actually costs.",
    ],
  },
  {
    version: "0.237.0",
  ...
  },
  {
    ...
    fixed: [
      ...
      "The attendee list given to the model when generating meeting minutes no longer includes a Multiple Speakers slot, so it cannot be written into the minutes as a person who attended.",
    ],
  },
  {
    version: "0.236.1",
  ...
  },
  {
    ...
    date: "2026-08-21",
    pr: 564,
    headline: "Join the Meeting now works for Teams invites",
    summary:
      "**Join the Meeting** was greyed out on every Microsoft Teams meeting, while working fine for Zoom. The difference was never Teams itself, it was where each service puts its link.\n\nA Zoom invite puts the join URL in the meeting's **location**, on its own, so finding it was as simple as taking the first web address in the invite. A Teams invite does the opposite. The location is the words \"Microsoft Teams Meeting\" with no address at all, and the join link sits in the body among four others that look just like it: a \"Need help?\" article, a dial-in page, the organiser's meeting options, and whatever the organiser pasted into the agenda above them. Taking the first address found the agenda link or the help article, and when the invite carried neither, the button simply stayed disabled.\n\nDiariz now recognises the services by name, so it picks the link that actually joins the meeting rather than the one that happens to come first. That covers both shapes of Teams link (including the short **teams.microsoft.com/meet** ones Teams hands out today, personal accounts, and government tenants), Zoom, Google Meet, Webex, Whereby and GoToMeeting. Links your mail system has rewritten for safety are recognised too, which is how most Teams invites arrive in a corporate mailbox.\n\nJust as important is what it will no longer open. A Join button that lands on a help article or a list of phone numbers is worse than no button at all, because you only find out it was not the meeting once the meeting has started. Those are now excluded outright.\n\nA meeting on a service Diariz has not heard of still behaves exactly as before, joining on the first link in the invite.",
    fixed: [
      "Join the Meeting is available on Microsoft Teams meetings. It was disabled on every one of them, because Outlook supplies no join link for a Teams appointment and the invite's location is the words \"Microsoft Teams Meeting\" rather than an address.",
      "The button no longer opens a Teams \"Need help?\" article, a dial-in page or the organiser's meeting-options page in place of the meeting.",
    ],
    changed: [
      "Join links are recognised by service - Teams (all forms), Zoom, Google Meet, Webex, Whereby and GoToMeeting - rather than by taking the first web address in the invite, so an agenda link pasted above the join block no longer wins. Anything else still uses the first address, as before.",
    ],
  },
  {
    version: "0.235.1",
  ...
  },
  {
    ...
    date: "2026-08-20",
    pr: 562,
    headline: "Test an AI model against one of your own recordings",
    summary:
      "The Run test button in the AI models editor now runs the real job against a real meeting, for the three call types where that makes sense: Tags, Actions and Summaries.\n\nUntil now every test sent the same thing: a made-up three-line transcript and one generic \"summarise this\" instruction, whichever tab you were on. That was deliberate - it keeps the timings comparable between models - but it only ever answered \"does this endpoint reply\". It could not tell you whether a model was any good at the job you had pointed it at. You found that out later, when real meetings came back with bad tags.\n\nPick one of your own recordings in the test panel and the call becomes the genuine one: the same prompt the platform sends, built from the same editable template file, over that meeting's actual transcript with its speaker names. The reply then runs through the platform's own parser, so you see what would really have been stored - tags with their weights, action items as a table with owner and deadline, a summary with the title it suggests - rather than a wall of raw text. The model's own words are still there, one click away.\n\nThe most useful result is the unhappy one. A model can answer confidently, spend tokens, and still produce nothing the platform can read; that used to look like a pass. It now says so plainly: \"the pipeline would have extracted no tags from this reply\".\n\nThe recording is remembered against your account, so all three tabs and every model you try afterwards are compared against the same meeting. Only your own recordings are offered, and only ones that have finished transcribing. Defaults, Minutes and formulas, Translation and Chat are unchanged and keep the built-in sample - Minutes is several calls rather than one, and Translation and Chat need a target language and a question that a recording does not supply.",
    added: [
      "A Test against picker in the model editor's test panel, listing your own transcribed recordings, remembered against your account and shared by the Tags, Actions and Summaries tabs.",
      "Test results for those three tabs are shown as the platform would have stored them: tags as weighted chips, action items as a table, a summary with its suggested name - with the model's raw reply behind a Raw reply toggle.",
      "A plainly worded note when a successful call would still have produced nothing usable, instead of an empty result that reads like a pass.",
    ],
    changed: [
      "Tags, Actions and Summaries tests send the real prompt for that call type over the chosen recording's transcript, rather than the built-in sample. Run test stays disabled on those tabs until a recording is chosen.",
      "Test calls now name the recording they ran against in the LLM usage log.",
    ],
  },
  {
    version: "0.234.0",
  ...
  },
  {
    ...
    date: "2026-08-20",
    pr: 557,
    headline: "Add every model on a server in one go",
    summary:
      "Adding the models on a server meant typing each one in by hand - its name, its endpoint, its key and its context window - which is tedious on a machine hosting a dozen of them and easy to get subtly wrong.\n\nThere is now an Add all button beside Add model on the AI models screen. Give it a server address and an optional key, press Discover, and it lists the chat models that server has. Embeddings, speech recognition and other non-chat models are left out. Models already configured are shown but cannot be picked again, so it is clear the server has them rather than looking as though they went missing.\n\nNothing is added until you press the button, which names how many you have ticked. Where a server does not report a model's context window - the plain OpenAI-compatible listing never does - the row says so and the model is added with a usable default of 16,384. That number decides how much of a meeting can be sent to the model, so the ones worth correcting are pointed out rather than left to be discovered later.\n\nImported models are not offered in chat until you tick them; adding forty models does not put forty entries in everyone's chat picker.",
    added: [
      "Add all on the AI models screen: point it at a server, see the chat models it has, and add the ones you want.",
      "Models whose context window the server did not report are flagged, so a defaulted value is visible rather than silent.",
    ],
  },
  {
    version: "0.231.0",
  ...
  },
  {
    ...
    date: "2026-08-19",
    pr: 555,
    headline: "Pages no longer break when Diariz is updated under you, and a recording is harder to lose",
    summary:
      "If Diariz had been left open across an update - which the desktop app does constantly, since closing the window only hides it to the tray - opening a page you had not visited yet in that session could fail with \"Something went wrong showing this page\" and a message about a module that could not be fetched. Manage AI models was the usual place to hit it, but every page that loads on demand was affected, and the only way out was to reload by hand.\n\nThe cause is that each update gives Diariz's files new names and removes the old ones. A window that was already open still asks for the old names, and they are no longer there. It looked like a desktop bug, but a browser tab left open across an update behaved the same way; browser tabs just get reloaded far more often.\n\nDiariz now notices this and reloads itself, so the page you asked for simply opens. If a recording is in progress it will not reload behind your back - it explains that an update is available and leaves the choice to you.\n\nWhile fixing that, we found nothing at all was protecting a recording from being thrown away. A live recording exists only in the open window until you press stop, so reloading, navigating away, closing the tab, or quitting the desktop app destroyed it without a word. Your browser now asks before leaving the page, and the desktop app asks before quitting or restarting to install an update. Neither interrupts you at any other time.",
    fixed: [
      "Opening a page after Diariz had been updated in the background no longer fails with a module-loading error. It reloads and shows the page instead.",
      "A reload is never triggered automatically while a recording or upload is in progress; you are offered the update and choose when to take it.",
      "Leaving the page - reloading, navigating away or closing the tab - while a recording or upload is running now asks first, instead of silently discarding it.",
      "Quitting the desktop app, or restarting it to install an update, now asks first if a recording is still running or uploading.",
    ],
  },
  {
    version: "0.230.1",
  ...
  },
  {
    ...
    date: "2026-08-18",
    pr: 545,
    headline: "Tell Diariz what language you are speaking",
    summary:
      "A two minute test recording in English came back as **Failed**, with nothing to show for it but the message *No default align-model for language: cy*. Whisper had decided the recording was in Welsh.\n\nIt guesses the language from the opening of the audio, before anything is known about whether there is speech there at all, so a recording that starts quiet is a coin toss. Welsh is one of the languages it lands on. And because Diariz has no word-level alignment model for Welsh, the guess did not merely mislabel the transcript - it threw the whole thing away, after the words had already been transcribed.\n\nTwo changes, either of which would have saved that recording.\n\n**You can now say what language a recording is in.** Re-transcribe a recording and the dialog offers the spoken language alongside the speaker hints, so a meeting that came back in the wrong language is one re-transcribe away from being right. If you always record in the same language, set it once under Preferences -> Profile as **Transcription language** and every new recording follows it, with any individual recording still free to differ. Leave either on **Detect automatically** and nothing changes from before.\n\n**A language with no alignment model no longer costs you the transcript.** Diariz aligns words for 37 languages; Whisper transcribes around 99. For the rest - Swedish, Thai, Tamil and Indonesian among them - a recording used to fail outright. Now it keeps the transcript, with speakers still identified and timings taken from the transcription itself rather than the word aligner. The only thing lost is a little precision at the edges of each segment.\n\nThe two work together: pinning the language stops the wrong guess, and the fallback means even a right guess in an unaligned language still gives you your meeting.",
    added: [
      "A **Transcription language** setting under Preferences -> Profile: the language your recordings are usually in, used unless a recording says otherwise.",
      "A spoken-language picker in the re-transcribe dialog, so a recording transcribed as the wrong language can be corrected.",
    ],
    fixed: [
      "A recording detected as a language Diariz has no alignment model for (Welsh, Swedish, Thai, Tamil, Indonesian and others) failed outright and produced nothing. It now keeps the transcript and the speaker labels, using the transcription's own segment timings.",
    ],
  },
  {
    version: "0.227.0",
    date: "2026-08-18",
    pr: 544,
    headline: "Auto-capture takes the screenshots for you",
    summary:
      "Sitting in a presentation, the way to get the slides into your transcript was to hit the screenshot hotkey on every one of them - and notice every time the presenter moved on. **Auto-capture** does it for you. Turn it on from the notes panel or the tray, and Diariz watches the capture area and takes a screenshot each time the screen settles on something new.\n\nThe word *settles* is doing the work. It watches once a second and only keeps a frame once the content has held still, so what you get is slides rather than everything that crossed the screen. A mouse cursor drifting over a slide is not a new slide. Neither is a blinking cursor, a half-drawn animation, or a fade between two slides. A video playing inside the deck produces nothing at all until the deck settles again. And going back to a slide you have already captured does not file a second copy of it.\n\nEach capture is timestamped with the moment its slide **appeared**, not the moment Diariz decided it had settled, so it lands in the transcript beside the sentence that introduced it rather than a few seconds after.\n\nIt looks after itself: it pauses when you pause the recording, stops if the meeting reaches its screenshot limit or the screen it was watching goes away, and always ends with the recording. It is never left running into your next meeting. Your hotkey, the tray item and the capture button all keep working while it runs, and anything it misses you can still grab by hand.\n\nOne thing worth knowing: a slide has to stay up for about three seconds to be captured. A deck flicked through faster than that will not be caught, which is deliberate - the alternative is a transcript full of half-drawn transitions.",
    added: [
      "Auto-capture: a toggle in the notes panel and the tray that screenshots the screen each time it changes, capturing a presentation slide by slide.",
    ],
    changed: [
      "Screenshots taken by auto-capture are timestamped from when the slide appeared, so they line up with what was being said about it.",
    ],
  },
  {
    version: "0.226.2",
  ...
  },
  {
    ...
    date: "2026-08-18",
    pr: 540,
    headline: "Take the test result somewhere else",
    summary:
      "A test result was a dead end. It told you what happened and then you were on your own - if you wanted to reproduce the call outside Diariz, or see it next to every other call the platform had made, you had to rebuild it by hand from what the editor showed you.\n\n" +
      "Every result now carries three ways out. **Copy as cURL** puts the exact request on your clipboard, ready to paste into a terminal: the endpoint that was called, the body that was sent, and the API key as a `$LLM_API_KEY` placeholder to substitute. It names the placeholder rather than dropping the header, so the command fails for an obvious reason if you forget rather than returning a 401 you then have to diagnose. **Raw JSON** copies the whole result. **Open in usage log** deep-links to that model's test calls, and a failure offers **Retry** instead.\n\n" +
      "That last one needed the usage log to learn to read a link at all. It now hydrates its filter from the query string, so `/admin/llm-usage?kinds=AdminTest&models=...` opens on the calls it names - which also means any filtered view is now linkable and pasteable. A link that has gone stale, been hand-edited, or been truncated by a chat client degrades to a broader view rather than an empty one or an error page: an unreadable date falls back to the usual week, and a call type that no longer exists is dropped. \"There is no usage\" is a far more misleading answer than \"here is the usual week\", because it reads as a fact about the platform rather than about the link.\n\n" +
      "The routing grid's **model column now stays put** while the call types scroll under it on a narrow screen. The dots mean nothing without the name they belong to, and the name was the first thing to leave the screen.",
    added: [
      "Copy as cURL, Raw JSON, Open in usage log and Retry on every test result.",
      "The LLM usage log reads its filter from the query string, so any filtered view can be linked to and pasted.",
    ],
    changed: [
      "The routing grid keeps the model column in view while the call types scroll horizontally.",
    ],
  },
  {
    version: "0.224.0",
    date: "2026-08-18",
    pr: 539,
    headline: "Ask the model, instead of guessing",
    summary:
      "Configuring a model was open-loop. You typed an endpoint, a key and thirteen parameters, pressed Save, and found out whether any of it was right the next time somebody's summary failed - by which point the failure was a line in the usage log with no obvious connection to the setting that caused it.\n\n" +
      "The model drawer now runs a real call. **Run test** sends one fixed sample request with whatever is on screen - saved or not - and reports what came back: how long the model took to produce its first token, how long the whole reply took, tokens per second, what it cost, and the reply itself. Each call type keeps its own result, because they ran with different parameters and are not comparable.\n\n" +
      "First-token time is separated from total duration on purpose. Together they answer a question the total alone cannot: a model that took nine seconds to start and one second to answer was loading, while one that started instantly and took ten seconds is simply slow. Those need opposite fixes, and until now both looked like \"the AI is slow\".\n\n" +
      "**A failure is where this earns its keep.** A timeout offers to raise the timeout for that call type. An endpoint that rejects a parameter by name offers to omit that parameter - and only for the call type you are looking at. That second one matters more than it sounds: *omitted* is the least obvious of the three parameter states and the hardest to discover, and the moment you need it is precisely the moment an endpoint has just refused a parameter. Offering it on the failure that calls for it is what finally makes the state legible.\n\n" +
      "Each row of the routing grid also has a **Test** button, with **Test all** in the footer. Those check reachability - the endpoint, the key, the model name - one model at a time, since several models commonly point at the same server and testing them together would measure that server's queue instead of the models.\n\n" +
      "Test calls appear in the usage log as **Model test**, so a call that spent tokens is never one you cannot account for. The reply itself is shown once and never stored: the usage log still records counts only.",
    added: [
      "Run test in the model drawer: a real call with the parameters currently on screen, saved or not, reporting first-token time, duration, tokens per second, token counts and the model's actual reply.",
      "One-click fixes on a failure - raise the timeout, or omit the parameter the endpoint rejected - applied to the open call type only.",
      "Test and Test all on the routing grid, checking each model's endpoint, key and model name one at a time.",
      "Test calls are recorded in the LLM usage log as Model test.",
    ],
  },
  {
    version: "0.223.0",
    date: "2026-08-18",
    pr: 538,
    headline: "One grid for what runs where",
    summary:
      "The AI models page had grown two answers to the same question. A table listed the models, and a second block underneath held eight dropdowns deciding which model ran which kind of call - so seeing what served summaries meant reading one list, then finding the matching row in another. The model editor had the opposite problem: seven panels of thirteen parameters stacked into a single dialog five thousand pixels tall, where the tab you wanted was somewhere below the fold.\n\n" +
      "Routing is now a grid. Models down the side, call types across the top, one selection per column - click a cell and that call type moves to that model. The dropdowns are gone, and so is the second block; what runs where is a shape you can take in at a glance rather than eight values you have to read.\n\n" +
      "A call type that follows the default model is a real state, not an absence, and it has a row of its own at the foot of the grid. That matters more than it sounds: a call type *following* the default moves with it when the default changes, while one *pointed at* the model that happens to be the default stays put. The old dropdowns could express both and showed them identically. The grid keeps them apart.\n\n" +
      "The editor is now a drawer with a tab per call type, each carrying a count of what it overrides, and the parameters in two columns rather than one long stack. Connection details - name, endpoint, key, context window - move behind a button, since they are set once and rarely touched.\n\n" +
      "The three parameter states are no longer three identical buttons. Typing in a parameter's box is what sets it; a loop button returns the row to inherited and a slashed zero omits it from the request. Which one is in force is now written under the row - \"overridden here\", or what it inherits and from where - rather than inferred from which of three buttons is filled in. **Inherited and omitted were the pair people confused**, and they no longer look alike.\n\n" +
      "Alongside the parameters, a panel shows the exact request body that call type would send, updating as you type. It is honest about what Diariz actually transmits: the timeout, tool and image flags, and the reasoning switch are summarised separately, because they control Diariz rather than the request and no endpoint ever receives them.",
    added: [
      "A routing grid on the AI models page: every model against every call type, one click to move a call type to another model.",
      "A No model row that returns a call type to following the default, and the default itself to the endpoint configured in the server environment.",
      "A request preview in the model drawer showing the exact body a call type would send, updated as parameters are typed.",
      "Inherited values now name what they inherit and from where, including the application defaults behind the Defaults tab.",
    ],
    changed: [
      "The model editor is a right-hand drawer with a tab per call type instead of a dialog stacking seven panels; connection details move behind a Connection button, and Delete model moves into the drawer.",
      "Setting a parameter means typing in its value box - the separate Inherit / Off / Set buttons are replaced by the field itself plus a return-to-inherited and an omit control, and each row states which state it is in.",
      "The models table and the separate assignments block are replaced by the grid.",
    ],
  },
  {
    version: "0.222.0",
    ...
    summary:
      ...
      "Every call now records the reason the model stopped, and the usage log shows a **Cut off** badge on any row where a token cap ended it. The outcome stays OK beside it, because the call genuinely did succeed - truncation is a separate signal, not a kind of failure, and treating it as one would both overstate the error rate and hide those rows behind the outcome filter. An operation is flagged when any of its calls was cut off, so the default view shows the problem without drilling in.\n\n" +
      "This is easiest to trigger on a reasoning model: reasoning is spent before the answer, so a cap that looks generous can be consumed entirely before a single word of the reply is written. The safest setting for the two token caps is to leave them alone, which sends no cap at all.\n\n" +
      "The model editor also gains its own help article, covering what each parameter does, why Off is not the same as Inherit, and the token-cap trap above.",
    added: [
      "A Cut off badge in the LLM usage log on any call or operation that stopped because it hit a token cap.",
      "The model's finish reason is recorded for every call, from both buffered and streaming responses.",
      "A help article for the model editor, reachable from the dialog itself.",
    ],
  },
  {
    version: "0.221.0",
    ...
    summary:
      ...
      "An administrator adds a model once - its name, endpoint, key and context window - and then tunes how it is called. Every sampling parameter is exposed, not just the handful that used to be hardcoded: temperature, top P, top K, repeat penalty, frequency and presence penalties, max tokens, max completion tokens, reasoning effort, the request timeout, and whether the model supports tool calling or image input. Each one can be inherited, set to a value, or switched off so it is left out of the request entirely - a real distinction, because some endpoints reject a parameter they do not understand.\n\n" +
      "Those parameters can differ by what the model is doing. Tag extraction, action extraction, summaries, minutes and formulas, translation, and chat each get their own optional overrides on top of the model's defaults, so a job that wants a cold, literal answer and a job that wants a fluent one can share a model without compromise. Reasoning effort is free text rather than a fixed list, because models disagree about what they accept - gpt-oss takes low, medium and high, qwen3 also takes xhigh, and the next one will take something else. A whole parameter set can be copied from another model as a starting point.\n\n" +
      "Different call types can also run on different models entirely: point tag extraction at something small and fast while summaries go to a larger model, with anything unassigned falling back to a chosen default.\n\n" +
      "**Nothing changes until you configure it.** An upgraded server keeps using the endpoint already set in its environment and sends exactly the same requests it sent before - the shipped defaults reproduce the previous behaviour, including translation's cooler setting. The models page offers to import that existing endpoint as your first model when you are ready.",
    added: [
      "An AI models page for Platform Administrators (Preferences, AI tab) listing every configured model, with a popout editor for its parameters.",
      "Every sampling parameter exposed per model: temperature, top P, top K, repeat penalty, frequency and presence penalties, max tokens, max completion tokens, reasoning effort, timeout, tool-calling and image support.",
      "Per-call-type parameter overrides for tag extraction, action extraction, summaries, minutes and formulas, translation, and chat.",
      "Assign a different model to each call type, with a default for anything unassigned.",
      "Copy a whole parameter set from one model to another.",
      "Free-text reasoning effort, so a model that takes xhigh (or anything else) can be configured.",
      "Application defaults for every parameter, overridable per deployment through LlmDefaults__* environment variables.",
    ],
    changed: [
      "AI model settings are removed from Preferences: the Assistant tab now covers chat tools only. The endpoint, key, model, context window, timeout and reasoning level are set by an administrator for the whole platform.",
      "The chat context dial reports the window of the model serving chat rather than a per-user figure, so the gauge and the actual truncation agree.",
    ],
  },
  {
    version: "0.220.0",
  ...
  },
  {
    ...
    added: [
      "A Tokens/s column in the Operations and Calls views of the LLM usage log, showing each row's own generation rate.",
      "The model time an operation actually spent (the sum of its calls' durations), distinct from the wall-clock span between its first and last call.",
    ],
  },
  {
    version: "0.218.0",
    date: "2026-08-17",
    pr: 533,
    headline: "A Platform Administrator page for the LLM usage log",
    summary:
      "The LLM usage log built in 0.216.0/0.217.0 has had no way to actually look at it until now - just three on/off settings. This release adds a Platform Administrator page at /admin/llm-usage, linked from the Model Settings tab, that opens the log up. Three views cover it from different angles: Operations groups every call into the user-facing action that triggered it (a chat turn, a summary, a formula run) with a turn count; Calls lists every individual model call; Summary rolls the same data up by any combination of user, model, and call type. A filter bar narrows any view by date range (last 7 days by default), user, call type, model, and outcome, and every column can be sorted, server-side, so the ordering is correct across the whole filtered set rather than just the page on screen. A totals row below the table always reflects the entire filter, not just the visible page - calls, operations, duration, and prompt/completion/reasoning/total tokens - and because not every call reports every token figure, each total states how many of the calls in scope actually measured it, so a partial number is never shown as if it were complete. An administrator can also delete rows matching the current filter, with a confirmation that states the exact row count before anything is removed. Nothing about the underlying log changes - this is a read (and delete) surface over data the platform was already capturing.",
    added: [
      "A Platform Administrator page at /admin/llm-usage (linked from Settings > Model Settings) with three views of the LLM usage log: Operations (one row per user-facing operation, with its turn count), Calls (every individual model call), and Summary (grouped by any combination of user, model, and call type).",
      "Filtering by date range (last 7 days by default), user, call type, model, and outcome, with server-side sorting on every displayed column.",
      "A totals row covering the whole filter, not just the visible page - calls, operations, duration, and prompt/completion/reasoning/total tokens - with each token total stating how many of the calls in scope actually reported that figure.",
      "Filtered deletion of usage log rows, with a confirmation stating the exact number of rows about to be removed.",
    ],
  },
  {
    version: "0.217.0",
    ...
    summary:
      "The LLM usage log added in 0.216.0 had a gap for streamed calls - chat replies and formula runs - which recorded no token counts and a duration of a few tens of milliseconds. That number was never the call: it was the time to the first response header, because a streamed reply keeps generating long after the response starts. This release closes the gap. When the streaming-token-count admin setting is on, the platform now asks the model endpoint for a token count on streamed responses too (stream_options.include_usage), and the usage-logging handler wraps the response body in a pass-through stream that forwards every byte to the browser untouched - nothing is buffered or delayed - while watching for the trailing usage event. The usage row is completed only when the stream actually ends, so it now carries prompt, completion, reasoning and total token counts, a true end-to-end duration, and a new time-to-first-token figure. A connection that drops or a read that faults mid-answer - including a closed tab or a Stop button, which cancels the in-flight read - is now attributed as a failure instead of being silently recorded as a success. Measured live on the same system before and after: a streamed chat turn with three model round-trips previously recorded 20 ms with no tokens; it now records 8,770 / 2,168 / 11,797 ms with token counts and a time-to-first-token of 1,896 / 324 / 2,935 ms.",
    added: [
      "Streamed chat replies and formula runs now report prompt/completion/reasoning/total token counts in the LLM usage log, when the streaming-token-count admin setting is on.",
      "Streamed calls now carry a time-to-first-token figure alongside the true end-to-end duration, instead of the old time-to-first-response-header number.",
    ],
    fixed: [
      "A streamed call's recorded duration used to measure only the time to the first response header (tens of milliseconds), not the actual generation time; it now measures the full call, completed when the stream ends.",
      "A connection that drops or a read that faults partway through a streamed reply - including a closed tab or a Stop button, which cancels the in-flight read - is now recorded as a failed call instead of a success.",
    ],
  },
  {
    version: "0.216.1",
  ...
  },
  {
    ...
    date: "2026-08-13",
    pr: 521,
    headline: "The upload dialog says \"Audio and video files\", and stops turning away files it can handle",
    summary:
      "The file dialog used to label its filter \"Custom Files\", which said nothing about what you could " +
      "pick. In Chrome and Edge it now says \"Audio and video files\"; Firefox and Safari do not let a " +
      "page name a filter, so they keep their own wording and offer the same formats. More usefully, the " +
      "dialog and the app no longer disagree with each other. The dialog offered any audio or video file, " +
      "but the app only accepted a fixed list of twelve extensions - so you could pick a file and then be " +
      "told it was an unsupported type, on a file that would have worked perfectly well. Ogg audio " +
      "(.oga), audiobooks (.m4b), camera and broadcast recordings (.ts, .m2ts, .mts) and phone video " +
      "(.3gp, .3g2) are now accepted, matching what the built-in converter has always been able to read.",
    added: [
      "Ogg audio (.oga), M4B audiobooks, MPEG-TS recordings (.ts, .m2ts, .mts) and 3GP/3G2 phone video are now accepted for upload.",
    ],
    changed: [
      "The upload dialog's file filter is now named \"Audio and video files\" in Chrome and Edge, instead of \"Custom Files\".",
    ],
    fixed: [
      "The upload dialog no longer offers files the app then rejects as an unsupported type.",
    ],
  },
  {
    version: "0.210.0",
  ...
  },
  {
    ...
    date: "2026-08-07",
    pr: 478,
    headline: "Turn on Outlook calendar sync",
    summary:
      "A new Outlook tab in Preferences, where you can switch on mirroring the calendar from classic Outlook " +
      "on your Windows PC. It is off until you turn it on, and the page says plainly what gets stored before " +
      "you do: meeting titles, times, locations, attendees and invite text are kept on the server, so those " +
      "meetings still work in a browser and once you have closed the desktop app. Private appointments are " +
      "skipped by default.\n\n" +
      "Each machine you connect is listed on its own, with when it last synced, how many meetings it holds, " +
      "and what went wrong if anything did - so a connection broken on your work PC is visible from your " +
      "laptop, or from any browser. You can rename a machine, hide it without disconnecting, change how far " +
      "back and forward it reads, or disconnect it entirely.\n\n" +
      "Two things are deliberately blunt. Disconnecting a machine deletes the meetings copied from it, and " +
      "turning the whole thing off deletes everything from every machine - both ask first. A privacy switch " +
      "that left the data it had gathered lying around would not be much of one.\n\n" +
      "The tab is there in any browser so you can read it, check your machines and revoke access from " +
      "anywhere. The syncing itself needs the Windows desktop app, which arrives in the next release.",
    added: [
      "An Outlook tab in Preferences with the opt-in, which is off by default and states what is stored.",
      "A per-machine list showing the mailbox, meeting count, last sync and last failure.",
      "Per-machine controls: rename, hide without disconnecting, how many days back and ahead to read, whether to skip private appointments, and whether to include invite text.",
      "Disconnecting a machine deletes the meetings copied from it; turning the opt-in off clears every machine. Both confirm first.",
    ],
  },
  {
    version: "0.189.0",
  ...
  },
  {
    ...
    date: "2026-08-06",
    pr: 457,
    headline: "Dragging a ticked row now moves everything you ticked",
    summary:
      "Multi-select and drag-and-drop disagreed: you could tick five recordings, drag one of them into a folder, and only the row under the cursor would move - the other four stayed where they were, with nothing to say so. A drag that starts on a ticked row now carries the whole selection, in the order the rows are shown rather than the order you ticked them, matching what Cut and Paste already did. Dragging a row that is not ticked still moves just that row, so an unrelated drag cannot sweep up a selection somewhere off screen. Alongside that, the folder path on a recording now leads with a folder icon (tinted to match that folder's colour in the list, so it reads as folders rather than tags) and sits directly under the recording's name instead of below the date line. The meetings list also gives back some vertical space around the search box and the folder path.",
    added: [
      "Dragging one of several ticked recordings moves the whole selection, keeping the order the rows are shown in.",
    ],
    changed: [
      "A recording's folder path now leads with a folder icon and sits directly under the name, above the source and date line.",
      "Tightened the spacing around the meetings list's search box and folder path, reclaiming vertical space for the list itself.",
    ],
  },
  {
    version: "0.184.0",
  ...
  },
  {
    ...
    date: "2026-07-29",
    pr: 371,
    headline: "A People screen, and speakers that say who they are",
    summary:
      "The last few releases built a people directory that only an API could see. This one puts it on " +
      "screen. **People** is now its own page rather than a tab tucked inside Preferences, which is where " +
      "a list shared by everyone belongs.\n\n" +
      "You can add someone who has never been recorded - a client you are about to meet - and give them a " +
      "job title, company, email address and phone number, and mark them internal or external. **Nobody " +
      "needs a voiceprint.** Diariz will simply not recognise them by voice, which is the point: holding " +
      "someone's contact details and holding their biometric are now separate decisions.\n\n" +
      "When two colleagues have each added the same person, the page says so and offers to merge them. It " +
      "never merges on its own - that deletes one of the two records and cannot be undone.\n\n" +
      "**Opting someone out of voice-printing** is a tick-box with a plain warning about what it destroys. " +
      "The state is always visible even to people who cannot change it, so nobody is left guessing. " +
      "Opting *yourself* out never needs a permission.\n\n" +
      "The Speakers tab on a recording now shows an identified speaker's job title, company, and whether " +
      "they are internal or external, so you can see who was in the meeting without leaving the transcript.",
    added: [
      "A **People** page listing everyone who appears in your meetings, with search and filters for internal, external, and whether a voiceprint is held.",
      "Editing a person's job title, company, email address, phone number and internal/external marker.",
      "**Possible duplicates** are pointed out on the page, by email address or by name, with a one-click merge.",
      "The Speakers tab shows an identified speaker's title, company and internal/external marker.",
      "A help article covering the directory, opting out, and merging duplicates.",
    ],
    changed: [
      "Voice Prints has moved out of Preferences to the People page - a directory shared by everyone is not a personal setting. Browsing it needs the Manage people permission.",
    ],
  },
  {
    version: "0.166.0",
  ...
  },
  {
    ...
    fixed: [
      "The recorder now clears its elapsed and auto-stop interval timers on unmount instead of leaving them running after you navigate away mid-recording",
    ],
  },
  {
    version: "0.149.0",
  ...
  },
  {
    ...
    changed: [
      "The folder view/manage authorization gates are now one shared implementation on IRoomScope instead of four near-duplicate copies across controllers - no behavior change",
    ],
  },
  {
    version: "0.146.1",
    ...
    summary:
      "A shared folder's Notes, Actions, and Attachments tabs aggregate rows from every recording in the folder, some of which belong to other room members. Edit, complete, and delete are owner-only on the server, but the folder page was offering those controls on every row regardless of whose recording it came from, so a co-viewer would click Edit or Delete and get an error. The page now checks each row's recording owner (the same check the recording page already uses) and only shows edit/delete/complete-toggle on rows you own; other rows still show their text, and a shared attachment link still opens (no permission needed to follow a URL), but its own edit/remove controls are hidden too. Separately, a folder's own attachments (filed directly against the folder, not aggregated from its recordings) were unusable in any shared room at all - every route required the folder to sit in the caller's personal room, so it 404'd even for whoever created it. That check now follows the same room-membership-plus-ManageContents rule folder create/rename/delete already use, and the Attachments tab's add/rename/remove/edit controls are hidden for a room member who lacks ManageContents (they can still open and read every attachment). Two follow-on issues in that widening are fixed here too: uploaded bytes now count against whoever uploaded the file, not the folder's creator, so a shared-room member could neither dodge their own quota nor fill someone else's by accident; and the Attachments tab now resolves its edit controls against the folder's actual room instead of whatever room a link's URL happens to name, so a folder link copied before this fix (or an old bookmark) can no longer show every control to a member who isn't allowed to use them. The same class of bug affected Formula results on both the recording page and the folder page's Formulas tab: the toolbar enabled Open/Download/Email/Delete purely on whether a result was selected, with no check against who created it or who can manage it, so a room member could be shown a Delete (or an editable Open) that the server would reject. Delete - and the Open modal's Save - are now gated per result against each side's own server rule (the folder side needs the creator or ManageContents in the folder's real room; the recording side, which has no room-permission check at all, needs the creator or the recording's owner); Open/Download/Email stay available to anyone who can already see the result, since those were never gated on the server either.",
    fixed: [
      ...
      "A folder's own attachments now count toward whoever uploaded them, not the folder's creator - a shared-room member could previously dodge their own quota, or unknowingly fill a colleague's, by uploading to a folder they didn't create",
      "The Attachments tab's add/rename/remove controls now resolve against the folder's actual room rather than the current page URL, so a folder link that carries no room (an older bookmark, or one copied before this fix) can't wrongly show every control to a member without permission",
      "The Formulas tab's Delete (and the Open modal's Save) on both the recording page and the folder page now only show for a result you created or are otherwise allowed to manage, instead of for any selected result",
    ],
  },
  {
    version: "0.146.0",
  ...
  },
  {
    ...
    date: "2026-07-11",
    pr: 263,
    headline: "Tidy up group membership on deletion",
    summary:
      "A behind-the-scenes correctness fix with no visible effect today. Room membership can be granted to a " +
      "user group, and because those are stored without a database link that cleans up automatically, deleting " +
      "a group used to leave its membership rows behind. Those leftover rows do nothing today, but they would " +
      "have quietly granted access once shared rooms gain members - so deleting a group now removes its " +
      "room-membership rows too (deleting a user already did).",
    fixed: [
      "Deleting a user group now also removes its room-membership rows, which the database could not clean up on its own.",
    ],
  },
  {
    version: "0.120.0",
