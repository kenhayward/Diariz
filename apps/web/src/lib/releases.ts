...
| **People and speaker identification** | One shared directory of the people in your meetings, where a voiceprint is optional - someone can be listed with no biometric held, or opt out, which erases theirs. Enrol a voice once and Diariz recognises it across later recordings (SpeechBrain voiceprints); rename, merge, and erase them (biometric data). Assign a speaker from the Speakers tab or straight from the label on any transcript row, as you read or listen. A **People** directory opens over whatever you are reading; it lists everyone one to a line, searchable by name, email or company; a person carries a job title, company, email, phone and an internal/external marker, and likely duplicates are pointed out for you to merge. The Speakers tab shows an identified speaker's title and company inline, with a contact card (email and phone as links) above their segments and a pencil to edit that person in place. Browsing the directory needs the Manage people permission; opting yourself out, and searching to name a speaker, never do. Your own entry is shown read-only on Preferences -> Profile - the name you appear under in transcripts, and whether a voiceprint is held for you. Each person's card has a **Voiceprint** tab: which recordings train their voiceprint, how much of each one's audio is behind it, and tick boxes to choose which segments to keep, re-embedding from exactly what you picked. Where two people share a name, every list shows which Diariz account each one is - or says plainly that there is none - so a merge is decidable. |
