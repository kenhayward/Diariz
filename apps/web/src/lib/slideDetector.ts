/// Slide-change detection for auto-capture. Pure: no DOM, no canvas, no timers. The caller owns the
/// frame source and the clock; everything here is decided from a stream of hashes, which is what lets the
/// whole state machine be tested against synthetic frames rather than against a screen.
///
/// The approach follows docs/Automatic_Slide_Capture.md: perceptual hashing, not pixel differencing. A
/// moving mouse cursor, a blinking text caret and codec noise all move a large number of pixels while
/// changing nothing a viewer would call a new slide, and only a structural hash survives them.
///
/// This lives in the web app rather than the desktop shell because detection runs in the renderer: the
/// frames come from a `getDisplayMedia` stream sampled onto a canvas, and the renderer is also the side
/// that owns the pause-aware recording clock. See §15 of the spec for why that beat doing it in the main
/// process (in short: 0.2% of a core instead of 43%).

/// dHash grid: (HASH_SIZE + 1) x HASH_SIZE samples -> HASH_SIZE^2 = 256 bits.
export const HASH_SIZE = 16;

/// Byte order of the frame handed to `dhash`. A canvas `ImageData` is RGBA; Electron's `nativeImage`
/// bitmap is BGRA.
export type PixelOrder = "bgra" | "rgba";

export interface SlideDetectorConfig {
  /// Distance above which the screen is considered to have changed at all.
  changeThreshold: number;
  /// Distance within which two consecutive samples count as "the same content, still there". Tighter
  /// than changeThreshold: this decides when something has *settled*.
  stabilityThreshold: number;
  /// Consecutive steady samples before a change is committed. See DEFAULTS for why this is 3.
...
export const DEFAULTS: SlideDetectorConfig = {
  changeThreshold: 24,
  ...
  /// multi-second transition sits on one intermediate digest for several samples in a row (measured: 25%
  /// and 50% through a fade hash identically). At 2 that intermediate accumulates a streak and a
  /// half-drawn frame commits as a slide - the exact failure this window exists to prevent.
  ///
...
/// A frame's bytes: whatever `ImageData.data` (Uint8ClampedArray) or a native bitmap hands over.
type Bytes = Uint8Array | Uint8ClampedArray;

/**
 * Difference hash of one sample. `bitmap` is 4-bytes-per-pixel image data for an image of exactly
 * (HASH_SIZE + 1) x HASH_SIZE - the caller is responsible for the resize, because the resize chain has to
 * be identical for every hash that will ever be compared.
 *
...
export function dhash(
  ...
  width: number,
  height: number,
  { pixelOrder = "bgra" }: { pixelOrder?: PixelOrder } = {},
): Uint8Array {
  // Frames arrive from two worlds with two byte orders (see PixelOrder). Reading one as the other still
  // produces a working hash, which is what makes it dangerous - it just weighs the colour channels
  // wrongly, so (say) a red-on-black chart and a blue-on-black one come out far more alike than a viewer
  // would call them.
...
function luma(bitmap: Bytes, i: number, rOffset: number, bOffset: number): number {
...
/// A change that has held steady long enough to be worth grabbing at full resolution.
export interface SlideCandidate {
  /// When the content first appeared - not when it was confirmed. This is what the capture is stamped
  /// with, so a slide lands on the sentence that introduced it rather than `stableSamples` later.
  firstSeenAtMs: number;
  hash: Uint8Array;
}

export interface ConfirmOutcome {
  /// False when the screen moved on during the grab: nothing is committed and the caller must discard
  /// the image it grabbed.
  accepted: boolean;
  /// True for a slide already captured this session (the presenter went back). It still becomes the
  /// committed state, but the caller should not file a second copy of it.
  duplicate: boolean;
}

export interface SlideDetector {
  observe(hash: Uint8Array, atMs: number): SlideCandidate | null;
  confirm(hash: Uint8Array): ConfirmOutcome;
  reject(): void;
  isDuplicate(hash: Uint8Array): boolean;
 ...
 */
export function createDetector(overrides: Partial<SlideDetectorConfig> = {}): SlideDetector {
  const cfg = { ...DEFAULTS, ...overrides };

  // The content currently believed to be on screen. Null until the first commit.
  let committedHash: Uint8Array | null = null;
  // The change in progress: what it looks like, when it first appeared, and how many consecutive samples
  ...
  const matchesCaptured = (hash: Uint8Array) =>
    captured.some((seen) => hamming(seen, hash) <= cfg.dedupeThreshold);

  return {
    /**
     * Take one sample. Returns null while nothing has settled, or a candidate the caller should now grab
     * and confirm.
     *
     ...
     */
    observe(hash, atMs) {
      // Nothing committed yet, so there is no "unchanged" to compare against - the first thing the screen
      // settles on is the first slide.
      if (committedHash !== null && hamming(hash, committedHash) <= cfg.changeThreshold) {
        // Back to what is already captured. Any change that was building was a transient - an overlay, a
        // notification, a transition frame - so it is dropped rather than left to accumulate a streak out
        // of unrelated samples.
        clearCandidate();
        return null;
      }

      if (candidateHash !== null && hamming(hash, candidateHash) <= cfg.stabilityThreshold) {
        streak++;
      } else {
        candidateHash = hash;
    ...
    confirm(hash) {
      ...
      const duplicate = matchesCaptured(candidateHash);
      // Adopt it either way. A repeat that was not adopted would leave the committed state stale, so
      // every later sample would read as a change and re-commit the same slide once per stability window
